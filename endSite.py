...
class Joint:
    def __init__(self, name: str, parent: Optional['Joint']):
        ...
        self.channels: List[str] = []  # ["Xposition","Yposition","Zposition","Yrotation","Xrotation","Zrotation"]
...
class BVH:
    ...
    def _parse(self):
        ...
        # ROOT
        assert lines[i].startswith("ROOT"); root_name = lines[i].split()[1]; i += 1
        ...
        assert lines[i].startswith("Frames:"); self.frames = int(lines[i].split()[1]); i += 1
        assert lines[i].startswith("Frame Time:"); self.frame_time = float(lines[i].split()[2]); i += 1
        ...
        for f in range(self.frames):
            ...
            if len(vals) != self.total_channels:
                raise ValueError(f"Frame {f} channel count mismatch: got {len(vals)} expected {self.total_channels}")
    ...
    def _local_tr(self, j: Joint, frame_vals: List[float]) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if j.channels:
            ...
            for k, ch in enumerate(j.channels):
                ...
                elif ch == "Xrotation": R = Rx(v) @ R 
                elif ch == "Yrotation": R = Ry(v) @ R 
                elif ch == "Zrotation": R = Rz(v) @ R 
                else:
...
if __name__ == "__main__":
    ...
    if not (pelvis_xyz.shape[0] == left_end_xyz.shape[0] == right_end_xyz.shape[0] ==
        ...
        raise ValueError("Pelvis / Left / Right / Shoulder의 프레임 수가 서로 다릅니다.")

    # [pelvis(3), L_shoulder(3), R_shoulder(3), left(3), right(3)] -> (N, 15)
    merged = np.hstack((pelvis_xyz,
                        left_shoulder_xyz, right_shoulder_xyz,
                        left_end_xyz, right_end_xyz))

    header = "pelvis_x,pelvis_y,pelvis_z," \
